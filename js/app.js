(function(){
    ...
    document.addEventListener("DOMContentLoaded", function() {
        glyphs.forEach(addGlyphOption);
        document.getElementById("glyphInput").addEventListener("input", glyphInputChange);
        words.forEach(w=>{document.body.append(addWordItem(w))});
    });
    const addGlyphOption = function(g){
        const datalist = document.getElementById("glyphs");
        const option = document.createElement("option");
        option.value = g.name;
        datalist.appendChild(option);
    }
    const addWordItem = function(w){
    ...
    const removeGlyphOption=function(g)
    {
    ...
    const glyphInputChange=function(e) {
        const input = e.target;
        const selected = glyphs.find(g => g.name === input.value);
        if (!selected) return;
        selectedGlyphs.push(selected);
        updateScoreDisplay();
        buildGlyphTable();
        updateWordDisplay();
        input.value='';
    ...
    const buildGlyphTable=function(){
    ...
    const buildGlyphCell=function(glyph, table){
    ...
    const calculateBaseScore=function(){
        return selectedGlyphs.map(g=> {return g.score}).reduce((a, c) => {return a + c}, 0)
    }
    ...
    const replaceCounter=function(rpnToken) {
        const match = rpnToken.match(/\[c(\d+)\]/gi);
        if (!match) return rpnToken;
        return selectedGlyphs.filter(t => t.score == parseInt(rpnToken.match(/\d/), 10)).map(i=>i.score).reduce((a, c) => {return a + c}, 0);
    };
    const replaceCounters=function(scoreString){
    ...
    const finalScoreCandidate=function(wordCombo){
        return rpn(replaceCounters(calculateBaseScore()+" "+wordCombo.map(w => w.action).join(" ")));
    }
    const allPossibleCombos = function()
    {
        const arr = words.filter(w => w.glyphs.every(wg => selectedGlyphs.some(g=>g.name==wg)));
    ...
    const updateWordDisplay=function(){
        ...
        selectedGlyphs.forEach(g => { document.querySelectorAll("[data-glyph='"+g.name+"']").forEach(el=>el.className="glyph set"); });
        selectedWords.forEach(w => {document.getElementById(w.glyphs.join("")).className="word selected"});
    }
